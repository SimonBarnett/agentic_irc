...
| `w-<shortid>-<pid>` worker | `#{machine}` only | Never `#bobiverse` (`w-fl-4412` → `#flamingo`; ionos: `w-io-<pid>` → `#ionos`). Spawn via `scripts/start_worker_irc_agent.py` (uses `bobreport.worker_irc_agent_args`) when the git worker PID is known — argv helper alone does not JOIN. |
