...
| `w-<shortid>-<pid>` worker | `#{machine}` only | Never `#bobiverse`. Ionos: `w-io-<pid>` → `#ionos`. Spawn helper: `scripts/start_worker_irc_agent.py` (UTF-8); callers are `agentic_build` `Start-BobCursor` + `Start-BobWorker` (fleet tick). |
