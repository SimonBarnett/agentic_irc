# Start or reuse irc_agent + irc_listen for a talk seat. Nick = {machine-id}-{agentPid}.
# coordinator.pid agent= is authoritative (never use listen= for the seat id).
param(
...
if ($nick -and $nick -ne $expectedNick) {
    Write-Output "INFO talk-seat nick=$nick expected=$expectedNick (agent PID is authoritative; not listen PID)"
    $nick = $expectedNick
}
if (-not $nick) {
    $nick = $expectedNick
...
$env:AGENTIC_IRC_DEBUG = '1'
& (Join-Path $Scripts 'Start-IrcTsr.ps1') -IrcHome $resolved -Scripts $Scripts | Out-Null
$listenPid = ''
