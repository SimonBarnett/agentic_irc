...
def test_peer_mismatch_does_not_overwrite(tmp_path: Path, monkeypatch):
    ...
    seal.genkey()
    c = irc_agent.Client(_args(tmp_path / "alice", "alice"))
    c.peers = {"bob": {"pk": "firstpkfirstpkfirstpkfirstpkfirstpk12", "nick": "bob"}}
    c.handle_privmsg("bob!u@h", "AGPK v1 otherpkotherpkotherpkotherpkotherpk12")
    assert c.peers["bob"]["pk"].startswith("firstpk")
