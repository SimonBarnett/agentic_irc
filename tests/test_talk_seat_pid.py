...
def test_resolve_seat_pid_from_coordinator_agent(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("AGENTIC_IRC_SEAT_PID", raising=False)
    home = tmp_path / "seat"
    home.mkdir()
    (home / "coordinator.pid").write_text(
        "nick=flamingo-19392\nagent=19392\nlisten=1\n", encoding="utf-8"
    )
    assert tsp.resolve_seat_pid(home) == 19392


def test_resolve_seat_pid_self(monkeypatch):
    monkeypatch.setenv("AGENTIC_IRC_SEAT_PID", "self")
    assert tsp.resolve_seat_pid(None, self_pid=55555) == 55555
